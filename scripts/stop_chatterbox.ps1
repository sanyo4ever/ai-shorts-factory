...
$logRoot = Join-Path $repoRoot "runtime\logs\chatterbox"
$latestPath = Join-Path $logRoot "latest.json"

function Get-ChatterboxPids {
    ...
    $matches = Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object {
        $_.CommandLine -like "*chatterbox_runner.py*"
    }
...
function Wait-ForChatterboxStopped {
    param(
        [int]$ListenPort,
        [int]$TimeoutSec = 15
    )
    ...
    while ((Get-Date) -lt $deadline) {
        if (@(Get-ChatterboxPids -ListenPort $ListenPort).Count -eq 0) {
            return $true
        }
...
$pids = @(Get-ChatterboxPids -ListenPort $Port)
if ($pids.Count -eq 0) {
    Write-Host "Chatterbox is not running on port $Port."
    exit 0
}

Stop-Process -Id $pids -Force
$stopped = Wait-ForChatterboxStopped -ListenPort $Port
...
if (-not $stopped) {
    Write-Warning "Chatterbox stop was requested but listener/process teardown was not confirmed within timeout."
}

Write-Host "Stopped Chatterbox PIDs: $($pids -join ', ')"
